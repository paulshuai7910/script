...
const MongoTracks = mongoose.model("tracks", tracksSchema)
const MongoUser = mongoose.model("users", userSchema)
async function connectDB() {
  try {
    await mongoose.connect(
      // 修改连接字符串
      "mongodb://dbReadOnlyForTracks:Len0vo%40R8n9@172.101.10.20:27017/fastgpt?authSource=admin&directConnection=true"
    )
    console.log("数据库连接成功")
  } catch (error) {
